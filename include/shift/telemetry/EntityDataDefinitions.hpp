...
inline constexpr DataDefinition<Entity, Vec3> EntityPosition{
    "position", [](const Entity& entity) -> std::optional<Vec3> {
        return entity.position();
    }};

inline constexpr DataDefinition<Entity, double> EntityPositionX{
    "position.x", [](const Entity& entity) -> std::optional<double> {
        return entity.position().x;
    }};

inline constexpr DataDefinition<Entity, double> EntityPositionY{
    "position.y", [](const Entity& entity) -> std::optional<double> {
        return entity.position().y;
    }};

inline constexpr DataDefinition<Entity, double> EntityPositionZ{
